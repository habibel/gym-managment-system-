...
<div class="row m-4 " style="height: 50px">
    <div class="col px-md-0 shadow pt-3 font-weight-bold font-italic rounded-pill" style="height: 50px;text-align:center;"> @if(session()->has('count')){{session()->get("count").' members'}} @endif</div>
...
<div class="row m-3 " style="height: 50px">
    <div class="col px-md-0" style="height: 50px"><button class="btn btn-info ml-5 "><a href="{{url('addemployee')}}" class="text-light"> add employee</a></button></div>
...
<table class="table table-striped">
    ...
    <tbody>
      @foreach ($all as $item)
      <tr>
<th scope="row" class="media-heading">{{ $item->name }}</th>
<td>{{ $item->prenom}}</td>
  <td>{{ $item->tel}}</td>
   <td>{{ $item->dateinscription}}</td>
  <td>{{ $item->gender}}</td>
  <td>{{ $item->saliare." DHs" }}</td>
  <td>
    <a href="{{ url('/editemployee/'.$item->id) }}" class="btn btn-warning">edit</a>
    <a href="{{ url('/deleteemployee/'.$item->id) }}"    onclick="deletecheck()" class="btn btn-danger">delete</a>
     <script>
